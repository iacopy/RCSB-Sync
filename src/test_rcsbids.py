...
def test_store_pdb_ids():
    ...
    :return: None
    """
    store_pdb_ids(EXPECTED_IDS_AF, 'src/test_pdb_ids.txt')
    with open('src/test_pdb_ids.txt', 'r', encoding='ascii') as file_pointer:
        ids = file_pointer.read().split('\n')
    # the last line should be empty (the file should end with a newline)
    assert ids[-1] == ''
    assert ids[:-1] == EXPECTED_IDS_AF
    os.remove('src/test_pdb_ids.txt')
...
def test_load_pdb_ids():
    ...
    """
    store_pdb_ids(EXPECTED_IDS_AF, 'src/test_pdb_ids.txt')
    ids = load_pdb_ids('src/test_pdb_ids.txt')
    assert ids == EXPECTED_IDS_AF
    os.remove('src/test_pdb_ids.txt')
...
def test__load_query():
    ...
    query = _load_query('src/test_query_csm.json')
